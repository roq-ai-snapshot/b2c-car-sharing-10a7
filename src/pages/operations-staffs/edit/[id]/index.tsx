...
import { useRoqClient, useOperationsStaffFindFirst } from 'lib/roq';
...
function OperationsStaffEditPage() {
  ...
  const { data, error, isLoading, mutate } = useOperationsStaffFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: OperationsStaffInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.operations_staff.update({
        data: values as RoqTypes.operations_staff,
        where: {
          id,
        },
      });
